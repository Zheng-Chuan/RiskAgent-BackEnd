...
## Phase 10 验证发现（2026-08-03）

Phase 10 的 5 分钟主动监控全链路验证已于 2026-08-03 完成，验证过程中收集到的实测数据为 RFC-006 描述的缺陷提供了直接证据.

### 信念累积问题实际暴露

在 5 分钟监控测试中，SystemEngineer 每 30 秒感知一轮（共 10 轮），每轮检测到 Redis 异常（Service scale to 0）都创建新的 Belief 和 Intention. 由于当前代码缺少信念去重机制（`_deliberate()` 不检查信念是否已处理过），**同一异常信号在 10 轮感知循环中被重复处理**，与上文「复现场景」描述的行为完全一致.

### LLM 调用频率实测数据

| 指标 | 实测值 | 说明 |
|------|--------|------|
| LLM 调用次数 | 133 次 / 5 分钟 | deepseek/deepseek-chat，真实调用（非 fallback） |
| LLM 调用频率 | 约 26.6 次/分钟 | 133 / 5 |
| 线性增长推算 | ~1596 次/小时 | 133 × 12 |
| 日调用推算 | ~38304 次/天 | 133 × 288 |

> **关键结论**：如果不做信念去重，LLM 调用次数会随监控运行时间线性增长. 当前 5min 验证（133 次）尚在 ProactiveBudget 预算范围内，但长时间运行（如 1 小时、1 天）会产生「信念风暴」，导致 LLM 成本失控.

### 与 Phase 10 验收标准的冲突分析

| 时间窗口 | LLM 调用次数（推算） | 预算状态 | 风险等级 |
|----------|---------------------|---------|---------|
| 5 分钟 | 133 次 | 在预算内 | 低 |
| 1 小时 | ~1596 次 | 接近超限 | 中 |
| 24 小时 | ~38304 次 | 远超预算 | **高** |

Phase 10 的 5min 验收标准（「LLM 总调用次数在预算范围内」）在短时间内可以通过，但 RFC-006 描述的信念累积缺陷意味着**长时间运行必然产生信念风暴**，这是从 5min 验证到 7×24 生产部署必须解决的前置问题.
...
| 基于实测数据估算去重后的 LLM 调用频率降低比例 | 待 Phase 14 压测验证 | 需要 Phase 14 性能验证提供去重前后的对照数据，初步估算去重可将重复信念的 LLM 调用降低 80%+ |
| 评估去重后对告警实时性的影响 | 待验证 | 信念去重会跳过已处理的相同信号，需确认不会延迟首次告警的投递时机 |
