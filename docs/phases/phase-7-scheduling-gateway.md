# Phase 7: 调度与多平台

## 状态

调度能力已完成. Gateway 正式承诺收敛为抽象层和统一路由

## 核心目标

让系统具备自主定时执行和多渠道交互能力, 为系统增加原生的定时任务能力并将系统从 MCP 单入口扩展为多通道统一适配架构.

## 时间盒与资源

- 时间：3-4 周
- 优先级：中

## 工作范围

### In Scope
- 内置调度系统 (方向十 §14.6)
- 多平台网关抽象层 (方向十一 §14.7)
- CronManager 核心实现
- 调度任务接入统一执行内核（生产未挂载，KI-001）
- 调度预算隔离
- 典型金融风控调度场景
- 网关适配器抽象层 (正式承诺仅保留核心契约和统一路由)
- 统一消息格式与路由

### Out of Scope（本期不做）
- 提示词缓存分层 (Phase 8)
- 前端可视化界面
- 移动端适配
- 非金融场景调度模板
- 具体企业通讯平台适配器作为正式交付承诺

## 当前状态说明

- `CronManager` 调度链路, 预算隔离, 场景模板已经在代码和测试中落地
- `GatewayAdapter` `GatewayMessage` `GatewayRouter` 抽象层和统一路由已落地
- 当前代码库仍保留 `SlackAdapter` 和 `WeChatWorkAdapter` 兼容实现
- 但从本轮开始, Phase 7 的正式能力口径收敛为 `GatewayAdapter` 抽象层和统一路由
- 具体平台适配器不再作为本 Phase 的正式验收项

## 详细 Checkpoint

### 方向十. 内置调度系统 (§14.6)

#### 目标

为系统增加原生的定时任务能力, 支持自然语言定义的周期性风控操作.

- [x] Checkpoint 14.5.1 CronManager 核心实现
  - 实现项: 新增 `src/riskagent_backend/scheduling/cron_manager.py`. 支持创建, 查询, 暂停, 删除定时任务. Cron 表达式和自然语言描述双模式. 每个 Cron 任务关联 `task_template` 和 `trigger_config`.
  - 验收方法: 运行 CronManager 单测和 3 个定时任务集成测试.
  - 验收证据: Cron 配置样例. 触发日志. 暂停和恢复记录.
  - 通过标准: Cron 按预期触发. 暂停后不触发. 恢复后继续触发.

- [~] Checkpoint 14.5.2 调度任务接入统一执行内核（部分实现，2026-09-03 审计由 [x] 降级）
  - 实现项: Cron 触发的任务统一走 `system_event -> ModeratorAgent -> TaskGraphExecutor` 主链. 不允许调度任务绕过治理体系.
  - 验收方法: 运行 2 个 Cron 触发的完整任务 case.
  - 验收证据: Cron 触发的 run_trace. event 和 moderator decision 记录. 统一执行链路日志.
  - 通过标准: Cron 任务进入统一执行内核. 有完整 trace. 受预算和审批约束.
  - 实际缺口（2026-09-03 复核）: CronManager / run_cron_triggered_workflow 库层实现与单测/集成测试完整，但 src/ 未实例化 CronManager、server.py 无调度器启动，生产链路不可用（KI-001）.

- [x] Checkpoint 14.5.3 调度预算隔离
  - 实现项: Cron 任务共享主动性预算 (ProactiveBudgetManager). 防止 Cron 递归创建无限任务. 单 Cron 失败不影响其他 Cron 和用户任务.
  - 验收方法: 运行 1 个 Cron 风暴模拟 case 和 1 个 Cron 失败隔离 case.
  - 验收证据: 预算限制日志. 递归防护记录. 隔离验证日志.
  - 通过标准: Cron 风暴被预算限制. 递归创建被阻断. 单 Cron 失败不影响其他任务.

- [x] Checkpoint 14.5.4 典型金融风控调度场景
  - 实现项: 预置至少 3 个金融风控场景的调度模板: `每日盘后风险汇总` `定时阈值巡检` `周度合规报告`. 模板可被用户自然语言配置覆写.
  - 验收方法: 运行 3 个场景模板的端到端 case.
  - 验收证据: 模板配置. 自然语言覆写记录. 完整 run_trace.
  - 通过标准: 3 个场景模板可正常触发并完成. 自然语言覆写生效.

### 方向十一. 多平台网关 (§14.7)

#### 目标

将系统从 MCP 单入口扩展为多通道统一适配架构, 当前正式目标聚焦于统一抽象层和路由能力.

- [x] Checkpoint 14.6.1 网关适配器抽象层
  - 实现项: 新增 `src/riskagent_backend/gateway/` 模块. 定义 `GatewayAdapter` 抽象基类, 包含 `receive_message()` `send_response()` `send_alert()` `platform_hints()`. 所有平台通过适配器映射到统一 `run_proactive_workflow()` 入口.
  - 验收方法: 运行适配器抽象层单测和 mock 平台集成测试.
  - 验收证据: 适配器接口定义. mock 测试通过记录.
  - 通过标准: 新增平台只需实现适配器. 核心逻辑无需修改.

- [x] Checkpoint 14.6.2 统一消息格式与路由
  - 实现项: 所有平台消息归一化为统一 `GatewayMessage` 格式. 路由层根据消息类型决定进入 `user_task` 还是 `system_event` 入口. 响应格式根据 `platform_hints` 适配.
  - 验收方法: 运行跨平台消息路由 case 和格式适配 case.
  - 验收证据: 归一化消息样例. 路由决策日志. 各平台适配后的响应格式.
  - 通过标准: 不同平台的同一请求产生相同的执行结果. 响应格式符合各平台规范.

## 风险与缓解

| 风险 | 概率 | 影响 | 缓解措施 |
|-----|------|------|----------|
| 调度过载消耗系统资源 | 中 | 高 | 共享 ProactiveBudgetManager + 递归防护 + 并发限制 |
| Cron 递归创建无限任务 | 低 | 高 | 递归深度检测 + 任务创建频率限制 |
| 平台 API 变更导致维护成本 | 高 | 中 | 适配器模式隔离变化 + 抽象接口稳定 |
| 多平台消息格式差异大 | 中 | 中 | 归一化消息格式 + platform_hints 适配策略 |

## 成功标准 (Exit Criteria)

- 实现 CronManager 和调度执行链路（库层完整，生产入口未挂载，见 KI-001）
- 调度任务接入统一执行内核（库层已实现、生产未挂载，见 KI-001）
- 实现调度预算隔离
- 实现 GatewayAdapter 抽象层
- 实现统一消息格式与路由
- 3 个金融场景调度模板端到端跑通（模板与库层测试完整，生产入口未挂载，见 KI-001）

## 交付物清单

- [x] 代码：CronManager, 调度预算隔离, GatewayAdapter 抽象层
- [x] 测试：Cron 单测和集成测试, 风暴模拟测试, 平台消息收发测试
- [ ] 文档：调度系统配置指南, 网关适配器开发指南（文档未产出, 全仓不存在此两份指南）
- [ ] 评测：3 个金融场景调度模板端到端 case（评测未产出, `grep -rni cron eval/` 零命中; 场景模板代码本身存在于 scheduling/cron_templates.py）

## 相关文档

- PRD：[docs/PRD.md](../PRD.md)
- 架构：[docs/ARCHITECTURE.md](../ARCHITECTURE.md)
